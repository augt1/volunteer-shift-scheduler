...
from shifts.models import ShiftVolunteer, Position, Shift
from events.models import Event
from django.db.models import Count, Sum, F, FloatField, Q, Min, Max
...
def volunteer_list(request):
    ...
    assigned_volunteers = Volunteer.objects.filter(available_positions__isnull=False).distinct().count()
    unassigned_volunteers = Volunteer.objects.filter(available_positions__isnull=True).count()
    ...
    volunteers = volunteers.annotate(
        ...
        total_hours=Sum(
            (ExtractHour('shifts__end_time') * 60 + ExtractMinute('shifts__end_time') -
             ExtractHour('shifts__start_time') * 60 - ExtractMinute('shifts__start_time')) / 60.0,
...
def update_volunteer_positions(request, pk):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'volunteers/partials/volunteer_positions.html', {
            'volunteer': volunteer,
            'positions': Position.objects.all()
        })
    
    return render(request, 'volunteers/partials/position_modal.html', {
        'volunteer': volunteer,
        'positions': Position.objects.all()
    })
...
def volunteer_schedule(request):
    ...
    return render(request, 'volunteers/volunteer_schedule.html', {
        'volunteers': volunteers,
        'days': event_dates,
